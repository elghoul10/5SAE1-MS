...
@Component({
  selector: 'app-event-list-dashboard',
  templateUrl: './event-list-dashboard.component.html',
  styleUrls: ['./event-list-dashboard.component.scss']
})
export class EventListDashboardComponent implements OnInit {
...
orders: Ordre[];
  ...
  addEvent(): void {
    ...
    if (this.validateEvent(this.newEvent)) {
      ...
      this.newEvent.orderTransportId = 1;
...
updateEventStatus(id: number, status: string): void {
  this.eventService.changeEventStatus(id, status, "Ghalia.hajtayeb@esprit.tn").subscribe({
